...
$j.extend($j.validator, {
	...
	attributeRules: function(element) {
		...
		for (var method in $j.validator.methods) {
			var value;
			// If .prop exists (jQuery >= 1.6), use it to get true/false for required
			if (method === 'required' && typeof $j.fn.prop === 'function') {
				value = $jelement.prop(method);
			} else {
				value = $jelement.attr(method);
			}
			if (value) {
				rules[method] = value;
			} else if ($jelement[0].getAttribute("type") === method) {
				rules[method] = true;
			}
		}
			...
			$j.ajax($j.extend(true, {
				...
				success: function(response) {
					...
					if ( valid ) {
						...
						errors[element.name] = previous.message = $j.isFunction(message) ? message(value) : message;
...
;(function($) {
	...
	if ( $j.ajaxPrefilter ) {
		$j.ajaxPrefilter(function(settings, _, xhr) {
		...
		$j.ajax = function(settings) {
			var mode = ( "mode" in settings ? settings : $j.ajaxSettings ).mode,
				port = ( "port" in settings ? settings : $j.ajaxSettings ).port;
			if (mode == "abort") {
				if ( pendingRequests[port] ) {
					pendingRequests[port].abort();
				}
