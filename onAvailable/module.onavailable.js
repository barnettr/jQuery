(function($j) {
  $j.extend({
    ...
    onContentReady: function(el, callback, obj, override) {
      $j.onAvailable(el, callback, obj, override, true);
    }

  });

  $j.extend($j.onAvailable, {
    ...
    checkAvailable: function() {
      ...
      for (var i = 0; i < listeners.length; ++i) {
         ...
         * $j.onAvailable listeners. If the DOM is ready or the next sibling
         * is detected, execute callbacks for all $j.onContentReady listeners.
         */          
        if (el && (!listener.checkContent || (listener.checkContent &&
          (el.nextSibling || el.parentNode.nextSibling || $j.isReady)))) {
